"""
Optuna hyperparameter search for U-Net glomeruli segmentation.
...
Each trial trains one U-Net from scratch.
The study can be backed by SQLite storage, useful for Colab restarts.
"""
...
class _PruningCallback(keras.callbacks.Callback):
    """
    Reports val_mean_io_u to Optuna each epoch.
    If Optuna decides the trial is bad, training stops early.
    """
...
def _objective(trial: optuna.Trial, args: argparse.Namespace) -> float:
    ...
    best_val_iou = max(
        history.history.get("val_mean_io_u", [float("-inf")])
    )

    model.save(str(trial_dir / "final_model.keras"))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Optuna hyperparameter search for U-Net."
    )

    parser.add_argument(
        "dataset_path",
        type=Path,
        help="Dataset root with train/ and validation/ folders.",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optuna storage URL, e.g. "
            "sqlite:////content/drive/MyDrive/unet_optuna.db"
        ),
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "models" / "unet_optuna",
        help="Where to save Optuna trial outputs.",
    )
